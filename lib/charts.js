 ...
 */
export class Chart {
  constructor (sequence = null, querent = 0, quesited = 3) {
    if (isArray(sequence)) {
      try {
    ...
    this.querent = this.getHouse(querent);
    this.quesited = this.getHouse(quesited);
  }
  ...
  getIndications (querent = this.querent, quesited = this.quesited, inCompany = false) {
    ...
    if (subQuerent) {
      ...
      console.log(`Stripping ${qIndex} from ` + JSON.stringify(springs.querent));
      springs.querent = springs.querent.filter(q => q.to != qIndex);
    }
    if (subQuesited) {
      // strip any springs to the original
      qIndex = this.quesited.index;
      console.log(`Stripping ${qIndex} from ` + JSON.stringify(springs.quesited));
