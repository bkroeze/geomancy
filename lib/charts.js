...
export class Chart {
  ...
  getIndications (querentIx = this.querent, quesitedIx = this.quesited, inCompany = false) {
    ...
    const indications = {};
    const work = {springs, occupations, conjunctions, mutations, translations, ...aspects};

    // add the weights to each item in the indications
    for (const [indicationType, values] of Object.entries(work)) {
      if (!has(INDICATION_WEIGHTING, indicationType)) {
        console.log('Cannot find weighting for', indicationType);
        indications[indicationType] = values;
      } else {
        ...
        try {
          ...
          console.log('Bad values', indicationType, values, e);
          system.exit();
        }
      }
    }
