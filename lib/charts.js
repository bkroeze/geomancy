...
export class Chart {
  ...
  getPerfections (querentIx = this.querent, quesitedIx = this.quesited) {
    ...
    const occupation = getOccupation(querent, quesited);
    const conjunction = getConjunctions(querent, quesited, houses);
    const springs = getSprings(querent, quesited, houses);
    const mutation = getMutations(springs, houses);
    const translation = getTranslations(querent, quesited, houses);
    const aspects = getAspects(querent, quesited, springs, houses);

    const perfections = {springs, occupation, conjunction, mutation, translation, aspects};

    //check for company, if this is not a subquery
    if (querentIx !== this.querent && quesitedIx !== this.quesited) {
      const querentCo = this.getCompany(querent);
      const quesitedCo = this.getCompany(quesited);
      let additional = [];
      if (querentCo) {
          additional.push(this.getPerfections(querentCo.house, quesitedIx));
          if (quesitedCo) {
            additional.push(this.getPerfections(querentCo.house, quesitedCo.house));
          }
      }
      if (quesitedCo) {
        additional.push(this.getPerfections(querentIx, quesitedCo.house));
      }
      additional.forEach(added => {addUnique(perfections, added);});
    }
    return perfections;
  }
...
function getAspects (initialQuerent, initialQuesited, springs, houses) {
  const aspects = {perfections: [], denials: []};
  if (springs.quesited) {
    springs.quesited.forEach((quesited) => {
      makeAspectsFor(aspects, initialQuerent, houses[quesited]);
    });
  }
  if (springs.querent) {
    springs.querent.forEach((querent) => {
      makeAspectsFor(aspects, houses[querent], initialQuesited);
    });
  }
  if (springs.querent && springs.quesited) {
    springs.querent.forEach((querent) => {
      springs.quesited.forEach((quesited) => {
        makeAspectsFor(aspects, houses[querent], houses[quesited]);
      });
    });
  }
  return aspects;
...
function getOccupation (querent, quesited) {
  ...
  if (querent.hasFigure(quesited)) {
    occupation.push(quesited.index);
  }
  return occupation;
}
...
function makeAspectsFor(aspects, querent, quesited) {
  ...
  if (querent.trines.indexOf(quesited.index) > -1) {
    aspects.perfections.push({ ...baseAspect, aspect: 'trine' });
  }
  if (querent.squares.indexOf(quesited.index) > -1) {
    aspects.denials.push({ ...baseAspect, aspect: 'square' });
  }
  if (querent.sextiles.indexOf(quesited.index) > -1) {
    aspects.perfections.push({ ...baseAspect, aspect: 'sextile' });
  }
  if (querent.opposition === quesited.index) {
    aspects.denials.push({ ...baseAspect, aspect: 'opposition' });
  }
}
