...
export class Chart {
  ...
  getPerfections () {
    ...
    const querentFig = (this.querent < 0 || this.querent > 11) ? {} : houses[this.querent].figure;
    const quesitedFig = (this.quesited < 0 || this.quesited > 11) ? {} : houses[this.quesited].figure;
    const springs = {querent: [], quesited: []};
    const occupation = querentFig.name === quesitedFig.name;
    for (let i = 0; i < 12; i++) {
